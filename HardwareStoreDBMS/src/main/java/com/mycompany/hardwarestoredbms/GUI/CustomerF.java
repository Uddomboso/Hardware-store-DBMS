...
public class CustomerF extends javax.swing.JFrame {
    ...
    private void setupUIEnhancements() {
        ...
        createInputCard();
        setupSearchBar();
        setTitle("Hardware Store - Customer Management");
        setMinimumSize(new Dimension(1200, 700));
    }
    
    private void enhanceTable() {
        customert.setRowHeight(28);
        customert.setSelectionBackground(new Color(70, 130, 180));
        customert.setSelectionForeground(Color.WHITE);
        customert.setGridColor(new Color(220, 220, 220));
        customert.setShowGrid(true);
        customert.setFont(new Font("Segoe UI", Font.PLAIN, 12));
        
        JTableHeader header = customert.getTableHeader();
        ...
        tableModel = (DefaultTableModel) customert.getModel();
        tableSorter = new TableRowSorter<>(tableModel);
        customert.setRowSorter(tableSorter);
        
        customert.setDefaultRenderer(Object.class, new javax.swing.table.DefaultTableCellRenderer() {
            @Override
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
                    boolean hasFocus, int row, int column) {
                Component c = super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
    ...
    private void enhanceButtons() {
        styleButton(jButton2, new Color(76, 175, 80), Color.WHITE, "Add Customer", "+");
        styleButton(jButton3, new Color(33, 150, 243), Color.WHITE, "Update Customer", "✎");
        styleButton(jButton4, new Color(239, 83, 80), Color.WHITE, "Delete Customer", "✕");
    ...
    private void createInputCard() {
        ...
        gbc.gridx = 1;
        jTextField2.setPreferredSize(new Dimension(250, 30));
    ...
    private void setupSearchBar() {
        ...
        searchField.setPlaceholder("Search by name, email, or ID...");
    ...
    private void showEmptyState() {
        if (tableModel.getRowCount() == 0) {
            JLabel emptyLabel = new JLabel("<html><div style='text-align: center;'><br><br><br>" +
                ...
                "<font size='4' color='#666666'><b>No Customers Found</b></font><br><br>" +
                "<font color='#999999'>Click the <b>+ INSERT</b> button to add a new customer</font></div></html>");
    ...
    private void insertCustomer() {
        ...
        try (Connection conn = Customer.getConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(query)) {
                ...
                stmt.setString(3, jTextField2.getText().trim());
            ...
            showErrorMessage("Failed to add customer: " + ex.getMessage());
        }
    }
    ...
    private void editCustomer() {
        ...
        if (jTextField4.getText().trim().isEmpty()) {
            showErrorMessage("Customer ID is required for update");
        ...
        try (Connection conn = Customer.getConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(query)) {
                ...
                stmt.setString(2, jTextField5.getText().trim());
                stmt.setString(3, jTextField2.getText().trim());
                stmt.setInt(4, Integer.parseInt(jTextField4.getText().trim())); 
            ...
            showErrorMessage("Failed to update customer: " + ex.getMessage());
        }
    }

    private void deleteCustomer() {
        if (jTextField4.getText().trim().isEmpty()) {
            showErrorMessage("Customer ID is required for deletion");
            highlightErrorField(jTextField4);
            return;
        }
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try (Connection conn = Customer.getConnection()) {
                ...
                try (PreparedStatement stmt = conn.prepareStatement(query)) {
                    stmt.setInt(1, Integer.parseInt(jTextField4.getText().trim())); 
                ...
                showErrorMessage("Failed to delete customer: " + ex.getMessage());
            }
        }
    }
    ...
    private void backupCustomers() {
        int confirm = JOptionPane.showConfirmDialog(this, 
            "Are you sure you want to backup all customers?", 
            "Confirm Backup", 
            JOptionPane.YES_NO_OPTION);
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try (Connection conn = Customer.getConnection()) {
                ...
                showSuccessMessage("Customers backed up successfully!");
            } catch (Exception ex) {
                showErrorMessage("Failed to backup customers: " + ex.getMessage());
            }
        }
    }
    ...
    private void restoreCustomers() {
        int confirm = JOptionPane.showConfirmDialog(this, 
            "Warning: This will replace all current customers with backup data.\nAre you sure?", 
            "Confirm Restore", 
            JOptionPane.YES_NO_OPTION, 
            JOptionPane.WARNING_MESSAGE);
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try (Connection conn = Customer.getConnection()) {
                ...
                showSuccessMessage("Customers restored successfully!");
                refreshTable();
            } catch (Exception ex) {
                showErrorMessage("Failed to restore customers: " + ex.getMessage());
            }
        }
    }
    ...
    private void initComponents() {
        ...
        customert.setBackground(new java.awt.Color(255, 255, 255));
        ...
        jMenu2.setFont(new Font("Segoe UI", Font.BOLD, 12));
